...
const Notifications = () => {
  ...
  // ======================================================
  // APPLY BUTTON CLICK – specific vacancy select
  // ======================================================
  const handleApplyClick = (item) => {
    // Save selected vacancy details so OnlineRegistration can read them
    localStorage.setItem("selectedNotificationId", String(item.id));
    localStorage.setItem("selectedApplicationFees", String(item.fees || ""));
    localStorage.setItem("selectedDepartment", item.department || "");
    // Also store applyFor so form can pre-select
    localStorage.setItem("selectedApplyFor", item.department || "");
  };
        ...
        <div className="container">
          <div className="row g-4">
            ...
            <div className="col-lg-9 col-md-8 col-12">
              <div className="card border-0 shadow-sm">
                <div className="card-header bg-white border-0">
                  <h3 className="mb-0">Notifications/Advertisements</h3>
                </div>
                        ...
                        {notifications.length === 0 ? (
                          ...
                          notifications.map((item, index) => (
                            <tr key={item.id || index}>
                              ...
                              {/* APPLY BUTTON – specific vacancy */}
                              <td>
                                <Link
                                  to={`/online-registration?notificationId=${item.id}&fees=${encodeURIComponent(item.fees || "")}&department=${encodeURIComponent(item.department || "")}`}
