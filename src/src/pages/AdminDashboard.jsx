...
//     try {
//       return JSON.parse(localStorage.getItem("vani_vacancies")) || [];
//     } catch {
//       return [];
//     }
//   });
...
//   // ======================================================
//   // VACANCY ADVERTISEMENT PDF
//   // ======================================================

//   const handleVacancyAdvertisement = (e) => {
//     const file = e.target.files[0];

//     if (!file) return;

//     if (file.type !== "application/pdf") {
//       alert("Please select a valid PDF advertisement.");
//       e.target.value = "";
//       return;
//     }
...
//       setVacancyForm((previous) => ({
//         ...previous,
//         advertisement: reader.result,
//         advertisementName: file.name,
//       }));
//     };
...
//   const uploadVacancy = (e) => {
//     e.preventDefault();

//     if (
//       !vacancyForm.department ||
//       !vacancyForm.fees ||
//       !vacancyForm.openDate ||
//       !vacancyForm.lastDate
//     ) {
//       alert("Please fill all vacancy details.");
//       return;
//     }

//     const newVacancy = {
//       id: Date.now(),
//       sr: vacancies.length + 1,
//       department: vacancyForm.department,
//       fees: vacancyForm.fees,
//       openDate: vacancyForm.openDate,
//       lastDate: vacancyForm.lastDate,
//       advertisement: vacancyForm.advertisement,
//       advertisementName: vacancyForm.advertisementName,
//       applyLink: vacancyForm.applyLink || "/online-registration",
...
//                 <button
//                   className={`btn text-start ${activeSection === "vacancies"
//                     ? "btn-warning"
//                     : "btn-outline-light"
//                     }`}
//                   onClick={() => {
...
//                     {activeSection === "vacancies" && "Upload Vacancy"}
...
//                             placeholder="Enter department / vacancy"
...
//                             name="applyLink"
//                             className="form-control"
//                             placeholder="/online-registration"
//                             value={vacancyForm.applyLink}
//                             onChange={handleVacancyChange}
//                           />
//                         </div>
...
//                             onChange={handleVacancyAdvertisement}
//                           />

//                           {vacancyForm.advertisementName && (
...
import {
  FaUsers, FaImages, FaVideo, FaBoxOpen, FaUserShield, FaSignOutAlt, FaBars, FaTimes,
  FaUserCheck, FaUserLock, FaTrash, FaUpload, FaPlus, FaEdit, FaFileAlt, FaEye, FaList, FaTag,
  FaTicketAlt, FaWarehouse, FaCreditCard, FaChartBar, FaShieldAlt, FaBriefcase,
} from "react-icons/fa";
...
const AdminDashboard = () => {
  ...
  const {
    videoGallery,
    addVideo,
    deleteVideo: deleteVideoFromDatabase,
  } = useVideoGallery();
  ...
  // Other Contexts
  const { users, fetchUsers, blockUser, unblockUser, deleteUser } = useUsers();
  const { categories } = useCategories();
  const { brands } = useBrands();
  const { coupons } = useCoupons();
  const { inventory } = useInventory();
  ...
  const resetGalleryForm = () => {
    setGalleryForm({ id: null, name: "", url: "" });
    setIsGalleryEditing(false);
  };
  ...
  const handleVideoSelect = (e) => {
    ...
    // Practical limit for MongoDB Base64 storage
    if (file.size > 10 * 1024 * 1024) {
      alert("Video size must be 10 MB or less.");
      e.target.value = "";
      return;
    }
    ...
    reader.onerror = () => {
      alert("Failed to read the selected video.");
    };
  ...
  const deleteVideo = async (id) => {
    ...
    try {
      const result = await deleteVideoFromDatabase(id);
  ...
  const addProduct = async (e) => {
    ...
    if (
      ...
      alert("Please fill Title, Price, Description and Image.");
      return;
    }
  ...
  // ======================================================
  // CANDIDATE HELPERS  (selected candidate delete works)
  // ======================================================
  const handleDeleteCandidate = (id) => {
    const confirmDelete = window.confirm(
      "Are you sure you want to delete this candidate?"
  ...
  const handlePaymentStatus = (candidate) => {
    const newStatus = candidate.paymentStatus === "Paid" ? "Unpaid" : "Paid";
    updatePaymentStatus(candidate._id || candidate.id, newStatus);
  };
  ...
  const uploadVacancy = (e) => {
    ...
    const updatedVacancies = [...vacancies, newVacancy];
    setVacancies(updatedVacancies);
    localStorage.setItem("vani_vacancies", JSON.stringify(updatedVacancies));
  ...
  return (
    <div className="min-vh-100" style={{ backgroundColor: "#f4f7fb" }}>
      {/* Mobile Header */}
      <div
        className="d-lg-none d-flex align-items-center justify-content-between p-3 text-white"
      ...
      <div className="container-fluid">
        <div className="row">
          {/* Sidebar */}
          <div
            className={`col-12 col-lg-3 col-xl-2 p-0 ${
          ...
          >
            ...
            >
              ...
              <div className="d-grid gap-2">
                ...
                >
                  <FaUsers className="me-2" /> Employee Management
                </button>
                ...
                >
                  <FaFileAlt className="me-2" /> Candidate Registration Details
                </button>

                {/* Contact Queries */}
                <button
                  className={`btn text-start ${
                    activeSection === "contactQueries"
                      ? "btn-warning"
                      : "btn-outline-light"
                  }`}
                  onClick={() => {
                    setActiveSection("contactQueries");
                    setSidebarOpen(false);
                  }}
                >
                  <FaFileAlt className="me-2" /> Contact Queries
                </button>
                ...
                >
                  <FaBriefcase className="me-2" /> Upload Vacancy
                </button>
                ...
                >
                  <FaImages className="me-2" /> Photo Gallery
                </button>
                ...
                >
                  <FaVideo className="me-2" /> Video Upload
                </button>
                ...
                >
                  <FaFileAlt className="me-2" /> Order Management
                </button>
                ...
                >
                  <FaList className="me-2" /> Category Management
                </button>
                ...
                >
                  <FaTag className="me-2" /> Brand Management
                </button>
                ...
                >
                  <FaTicketAlt className="me-2" /> Coupon Management
                </button>
                ...
                >
                  <FaShieldAlt className="me-2" /> Security Management
                </button>
              </div>
          ...
          <div className="col-12 col-lg-9 col-xl-10 p-3 p-md-4">
                            ...
                            <td>
                              <div className="d-flex gap-2">
                                ...
                                >
                                  {employee.status === "Active" ? (
                                    <>
                                      <FaUserLock className="me-1" /> Block
                                    </>
                                  ) : (
                                    <>
                                      <FaUserCheck className="me-1" /> Unblock
                                    </>
                                  )}
                                </button>
                                <button
            ...
            {/* Candidates – selected candidate delete works */}
            {activeSection === "candidates" && (
              <CandidateManagement
                candidates={candidates}
                selectedCandidate={selectedCandidate}
            ...
            {activeSection === "vacancies" && (
              <>
                <div className="card border-0 shadow-sm mb-4">
                  <div className="card-body p-3 p-md-4">
                    <h5 className="fw-bold mb-4">Upload New Vacancy</h5>
                    <form onSubmit={uploadVacancy}>
                      <div className="row g-3">
                        <div className="col-12">
                          <label className="form-label fw-semibold">
                        ...
                        <div className="col-12">
                          <button type="submit" className="btn btn-success px-4">
                            <FaUpload className="me-2" /> Upload Vacancy
                          </button>
                        </div>
                      </div>
                    </form>
                ...
                <div className="card border-0 shadow-sm">
                  <div className="card-body">
                    <h5 className="fw-bold mb-4">Uploaded Vacancies</h5>
                    {vacancies.length === 0 ? (
                      <div className="text-center py-5">
                        <FaBriefcase
                          className="text-muted"
                        ...
                        <div className="col-12">
                          <div className="d-flex flex-wrap gap-2">
                            <button type="submit" className="btn btn-success">
                              {isGalleryEditing ? (
                                <>
                                  <FaEdit className="me-2" /> Update Photo
                                </>
                              ) : (
                                <>
                ...
                <div className="card border-0 shadow-sm mb-4">
                  <div className="card-body p-4">
                    ...
                    <small className="text-muted d-block mb-3">
                      Supported: MP4, WebM, OGG, MOV | Maximum size: 10 MB
                    </small>
                  ...
                  {videoGallery.length === 0 ? (
                    ...
                    videoGallery.map((video, index) => {
                      ...
                      const videoUrl =
                        video.url ||
                        video.path ||
                        `${API_URL}/videos/video/${video._id || video.id}`;
                    ...
                    >
                      <div className="card h-100 border-0 shadow-sm">
                        ...
                        <div className="card-body">
                          ...
                          <button
                            className="btn btn-outline-danger btn-sm mt-3 d-block"
                            onClick={() =>
              ...
              <div className="card border-0 shadow-sm">
                <div className="card-body">
                  <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
                    <div>
                      <h5 className="fw-bold mb-1">Contact Queries</h5>
                      <small className="text-muted">
                        Queries submitted from the Contact page
                      </small>
                    </div>

                    <div className="d-flex align-items-center gap-2">
                      <span className="badge bg-primary fs-6">
                        {contactQueries.length} Queries
                      </span>
                      ...
                      <button
                          ...
                          try {
                            ...
                            if (response.ok && data.success) {
                              ...
                              alert(
                                data.message ||
                                  data.error ||
                                  "Failed to refresh contact queries."
                              );
                            }
                          } catch (error) {
                            console.error(
                              "Refresh contact queries error:",
                              error
                            );
                            alert(
                              "Failed to refresh contact queries."
                        ...
                        disabled={contactQueriesLoading}
                      >
                  ...
                  {contactQueriesLoading ? (
                    ...
                    <div className="text-center py-5">
                      ...
                      <p className="text-muted mb-0">
                        Contact page submissions will appear here.
                      </p>
                    </div>
                  ) : (
                    <div className="table-responsive">
...
// ======================================================
// CANDIDATE MANAGEMENT  (delete selected candidate works)
// ======================================================
const CandidateManagement = ({
  candidates,
  selectedCandidate,
      ...
      <div className="card-body">
        ...
        {selectedCandidate && (
          <CandidateFullDetails
            candidate={selectedCandidate}
            onBack={() => setSelectedCandidate(null)}
            onDelete={() => onDelete(selectedCandidate._id || selectedCandidate.id)}
          />
        )}
      </div>
    </div>
...
// ======================================================
// FULL CANDIDATE DETAILS  (delete button also here)
// ======================================================
const CandidateFullDetails = ({ candidate, onBack, onDelete }) => {
  return (
    <div className="candidate-admin-details">
      <div className="d-flex flex-wrap justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Candidate Registration Details</h4>
        <div className="d-flex gap-2">
          ...
          >
            <FaTrash className="me-1" /> Delete Candidate
          </button>
        </div>
      </div>
      ...
      <div className="row g-3 mb-4">
        ...
        <InfoBox
          ...
          value={candidate.registrationDate}
        />
        <InfoBox label="Payment Status" value={candidate.paymentStatus} />
      </div>
